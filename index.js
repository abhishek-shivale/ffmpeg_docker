...
const processRendition = (rendition, index) => {
  return new Promise((resolve, reject) => {
    ...
    masterContent += `#EXT-X-STREAM-INF:BANDWIDTH=${parseInt(rendition.videoBitrate) * 1000},RESOLUTION=${rendition.resolution}\n${rendition.name}/playlist.m3u8\n`;
    ...
    ffmpeg(inputPath)
      .outputOptions([
        ...
        `-vf scale=${rendition.resolution.split('x')[0]}:${rendition.resolution.split('x')[1]}`,
      ...
      .on('progress', p => console.log(`Processing ${rendition.name}: ${p.percent.toFixed(2)}%`))
      .on('end', () => {
